    ...
    def view_pokemon(self):
        ...
        for id, pokemon in enumerate(self.belt):
            ...
            if (pokemon.is_ko == True):
                ko_status = '- fainted!'
            print('{0} - {1} Level {2} {3}/{4} HP'.format(id + 1, pokemon.name, pokemon.lvl, round(pokemon.current_health,2),
...
def show_belt(player, pokemon):
    ...
    print('\n{0}    Level {1}   {2}/{3} HP'.format(pokemon.name, pokemon.lvl, round(pokemon.current_health,2), int(pokemon.max_health)))
...
def wild_battle():
    ...
    while (player.own_pokemon() and not opponent_pokemon.is_ko and not caught):
        print("\n{0}    Level {1}   {2}/{3} HP".format(opponent_pokemon.name, int(opponent_pokemon.lvl),
                                                    round(opponent_pokemon.current_health,2), int(opponent_pokemon.max_health)))
        print()
...
def trainer_battle():
    ...
    if not player.own_pokemon():
        ...
        print('{0} has defeated {1} and has gotten {2} for winning.'.format(player.name,opponent.name,opponent.wallet))
...
if __name__=='__main__':
    ...
    time.sleep(6)
    print('--You look around in dread realising that you put too much effort into your outfit \nand that you completely forgot to bring any POKEMON with you!--')
    time.sleep(6)
    print('--As sweat drips from your forehead you notice several Pokeball laying on the table \nand you grab them before the professor'
          ' notices anything.--')
